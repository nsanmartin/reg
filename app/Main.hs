...
go StdInput  = do
    ...
    let nregs = length regList
    let newRegs = take nregs $ lines newContents
    putStr (unlines $ toRegScreen newRegs)
    let updatedRegs = take nregs $ lines $ newContents ++ oldContents 
